...
var skills =_.sortedUniq(_.sortBy(_.map(_.flatten(_.map(newObj, 'skills')), _.toLower)));
console.log("1. Массив скиллов (поле skills) всех людей, не должно быть повторяющихся скиллов, так же они должны быть отсортированы по алфавиту:", skills);

var personObj = _.map(newObj, function(newObj) {
  return {"name" : newObj.name, "friends" : newObj.friends.length};
});

var objNew = _.map(_.orderBy(personObj, ["friends", "name"], ['asc', "asc"]), "name");
console.log("2. Массив имен (поле name) людей, отсортированных в зависимости от количества их друзей (friends): \n", objNew);

var allFriend = _.sortedUniq(_.sortBy(_.map(_.flatten(_.map(newObj, 'friends')), "name")));
